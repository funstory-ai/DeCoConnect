...
async function init() {
  ...
  if (!document.querySelector('div[class^=pc_container]').children[0]
  ) {
    console.log('retry');
    window.janusAlreadyInit = false;
  ...
  observer.observe(document.querySelector('div[class^=pc_container]').parentElement, { childList: true });
